...
export async function POST(req: Request) {
    try {
        ...
        const { breakStart, breakEnd } = body as { breakStart: string; breakEnd: string };

        if (!breakStart || !breakEnd) {
            return NextResponse.json({ error: "Missing breakStart or breakEnd" }, { status: 400 });
        }
        ...
        if (breakStartMs >= breakEndMs) {
            return NextResponse.json({ error: "breakStart must be before breakEnd" }, { status: 400 });
        }

        const now = new Date();
        if (breakEndMs > now.getTime()) {
            return NextResponse.json({ error: "breakEnd cannot be in the future" }, { status: 400 });
        }
                ...
                prisma.workLog.update({
                    ...
                    data: {
                        ...
                        totalHours: parseFloat(
                            ((breakStartMs - activeLog.punchIn.getTime()) / 3_600_000).toFixed(4)
                        ),
                        status: "completed",
                    },
                }),
