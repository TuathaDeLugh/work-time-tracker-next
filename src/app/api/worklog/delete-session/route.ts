    ...
    try {
        ...
        if (action === "delete-break") {
            ...
            if (!previousLogId || !nextLogId) {
                return NextResponse.json({ error: "Missing previousLogId or nextLogId" }, { status: 400 });
            }
            ...
            if (!prevLog || !nextLog) {
                return NextResponse.json({ error: "One or both logs not found" }, { status: 404 });
            }
        ...
        return NextResponse.json({ error: "Invalid action. Use 'delete-break' or 'delete-work'." }, { status: 400 });
    } catch (error) {
